...
make_stats_table <- function(df) {
    N <- nrow(df)
    gather(df, factor_key = TRUE, na.rm = TRUE) %>%
        group_by(key) %>%
        summarize(n_present = n(),
                  perc_present = 100 * n_present / N,
                  min = ifelse(n_present == 0, NA, min(value)),
                  max = ifelse(n_present == 0, NA, max(value)),
                  med = median(value),
                  mean = mean(value),
                  stdev = sd(value),
                  range = max - min) %>%
...
df_y <- readr::read_tsv(path, col_types = cols(label = "c", .default = "-")) %>%
    pull(label)
