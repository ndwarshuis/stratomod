...
truncate_maybe <- function(df, name) {
    ...
    list(df = .df, lower = lower, upper = upper)
}

make_integer_plot <- function(df, xlab, lower = NULL, upper = NULL) {
...
print_uv_plot <- function(vartype, df, name) {
    p <- if (vartype == "continuous") {
             ...
             if (t == "step") {
                 make_continuous_plot(tr[["df"]], name)
             } else if (t == "bar") {
                 make_integer_plot(tr[["df"]], name, tr[["lower"]], tr[["upper"]])
             } else {
...
print_cont_cat_plot_inner <- function(df, cat_name, cont_name) {
    ...
    p <- if (t == "step") {
             ggplot(tr[["df"]], aes(value, score)) +
                 geom_step(aes(y = lower), color = "red") +
                 geom_step(aes(y = upper), color = "red") +
                 geom_step() +
                 xlab(cont_name)
         } else if (t == "bar") {
             make_integer_plot(tr[["df"]], cont_name, tr[["lower"]], tr[["upper"]])
         } else {
