...
## TODO need to pull in the training dataframe here as well to get the labels
## right
...
make_continuous_fraction_plot <- function(df, name, caption = NULL) {
    p <- df %>%
        mutate(value = cut(value, 20, labels = F)) %>%
        group_by(value) %>%
        summarize(frac = mean(label),
                  stderr = sqrt(frac * (1 - frac) / n())) %>%
...
make_categorical_fraction_plot <- function(df, name) {
    df %>%
        ...
        summarize(frac = mean(label),
                  stderr = sqrt(frac * (1 - frac) / n())) %>%
        ggplot(df, aes(factor(value), frac)) +
        geom_col() +
        geom_errorbar(aes(ymin = frac - stderr, frac + stderr), width = 0.1) +
        xlab(name) +
        ylab("Frac(TP)")
}
    ...
    r <- if (vartype == "continuous") {
             ...
             fs <- if (t == "step") {
                       list(
                           ...
                           partial(make_continuous_fraction_plot, caption = tr[["caption"]]))
                   } else if (t == "bar") {
                       list(
                           partial(make_integer_plot,
                                   ...
                                   upper = tr[["upper"]],
                                   caption = tr[["caption"]]),
                           partial(make_integer_fraction_plot,
                                   lower = tr[["lower"]],
                                   upper = tr[["upper"]],
                                   caption = tr[["caption"]])
                       )
             ...
             list(
                 feat_f = wrap_split_maybe(name, f[[1]]),
                 frac_f = f[[2]]
                 df = tr[["df"]]
             )
             ...
             list(
                 ...
                 frac_f = make_categorical_continuous_plot,
                 df = df
             )
         } else {
