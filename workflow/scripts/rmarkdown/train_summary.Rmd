...
alltrain <- bind_cols(train_x, train_y)
...
make_integer_fraction_plot <- function(df, name, caption = NULL) {
    .name <- sym(name)
    p <- df %>%
        rename(value = {{ .name }}) %>%
...
make_categorical_fraction_plot <- function(df, name) {
    ...
    df %>%
        ...
        summarize(frac = mean(label),
                  stderr = sqrt(frac * (1 - frac) / n())) %>%
        ggplot(aes(factor(value), frac)) +
        geom_col() +
        geom_errorbar(aes(ymin = frac - stderr, ymax = frac + stderr), width = 0.1) +
        xlab(name) +
        ylab("Frac(TP)")
}
...
print_uv_plot <- function(vartype, df, name) {
    r <- if (vartype == "continuous") {
             ...
             list(
                 feat_f = wrap_split_maybe(name, fs[[1]]),
                 frac_f = fs[[2]],
                 df = tr[["df"]]
             )
